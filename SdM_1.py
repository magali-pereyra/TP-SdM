...
class CarreraInvalidaError(Exception):
    pass
...
class Persona:
    ...
    def __init__(self, nombre, apellido,carrera):
        ...
        if carrera not in CARRERAS_DISPONIBLES:
                    raise CarreraInvalidaError(f"Carrera inválida: {carrera}")
...
class PlanDeEstudio:
    ...
    """
    def __init__(self, nombre_plan, ruta_json,carrera, creador: Persona):
        """
        ...
        if carrera not in CARRERAS_DISPONIBLES:
            raise CarreraInvalidaError(f"Carrera inválida: {carrera}")
        if creador.carrera != carrera:
            raise PermissionError(f"{creador.nombre_completo()} no tiene permisos para crear un plan para {carrera}")
        self.carrera = carrera
        self.materias = []
...
class Curso:
    ...
    def __init__(self, codigo_uc, año, semestre, carrera, plan: PlanDeEstudio, creador):
        """
        ...
        """
        if creador.carrera != plan.carrera:
            raise PermissionError(f"{creador.nombre_completo()} no tiene permisos para crear cursos para {plan.nombre_plan}")
        ...
        try:
            ...
            self.estudiantes = []
            self.archivo_csv = f"curso_{self.carrera}_{self.codigo_uc}_{self.año}_sem{self.semestre}.csv"
...
class InstanciaDeExamen:
    ...
    """
    def __init__(self, codigo_uc, fecha, hora, carrera, plan: PlanDeEstudio, creador):
        """
        ...
        """
        if creador.carrera != plan.carrera:
            raise PermissionError(f"{creador.nombre_completo()} no tiene permisos para crear instancia de examen para {plan.nombre_plan}")
    ...
    def generar_csv(self):
        ...
        archivo_csv = f"examen_{self.carrera}_{self.codigo_uc}_{self.fecha}.csv"
...
class Estudiante(Persona):
    ...
    """
    def __init__(self, nombre, apellido, cedula, carrera, año_ingreso, plan: PlanDeEstudio):
        """
        ...
        """
        super().__init__(nombre, apellido,carrera)
        self._cedula=cedula
        try:
            cedula = int(cedula)
...
class Coordinadora(Persona):
    ...
    def crear_plan(self, nombre_plan, ruta_json,carrera,creador):
        """
        ...
        """
        plan = PlanDeEstudio(nombre_plan, ruta_json,carrera,creador)
        print(f"{self.nombre} ha creado el plan '{nombre_plan}'.")
        return plan
        
    def inscribir_a_examen(self, estudiante, carrera,plan,examen,creador):
        """
    ...
    def crear_curso(self, codigo_uc, año, semestre, plan,carrera,creador):
        """
        ...
        curso = Curso(codigo_uc, año, semestre, plan,carrera,creador)
